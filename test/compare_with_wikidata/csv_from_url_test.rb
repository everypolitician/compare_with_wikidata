...
describe 'CompareWithWikidata::CSVFromURL' do
  def csv_from_url(url)
    CompareWithWikidata::CSVFromURL.new(url).parsed
  end
  ...
  it 'should produce a sensible response from genuine CSV' do
    ...
    csv_from_url('http://example.com/real.csv').must_equal [
  ...
  it 'should produce a helpful exception message on a 404 error' do
    ...
    error = assert_raises Exception do
      csv_from_url('http://example.com/non-existent')
    end
  ...
  it 'should produce a helpful exception message on a 404 error' do
    ...
    error = assert_raises Exception do
      csv_from_url('http://example.com/errors')
    end
...
'
    ...
    csv_from_url('http://example.com/html-but-also-valid-csv.csv').must_equal [
...
'
    ...
    error = assert_raises CompareWithWikidata::MalformedCSVError do
      csv_from_url('http://example.com/not-really-csv.csv')
    end
